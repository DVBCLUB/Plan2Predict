...
import { Calculator, CheckCircle2, ClipboardList, Code2, Copy, FileText, Gauge, Play, ReceiptText, ShieldAlert, Sparkles, TrendingUp, WalletCards, Wrench } from 'lucide-react';
...
const tools: ToolItem[] = [
  ...
  { id: 'journal_simulator', order: '02', title: 'Journal Entry Simulator', subtitle: 'Mô phỏng bút toán Nợ/Có và ảnh hưởng báo cáo', icon: FileText, badge: 'VAS' },
  ...
  { id: 'document_checklist', order: '10', title: 'Document Checklist Generator', subtitle: 'Sinh checklist hồ sơ theo loại nghiệp vụ', icon: ClipboardList, badge: 'DOC' }
];
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-2">
              🧰 Interactive Simulator Hub
              <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 text-[9px] font-black rounded font-mono">TOOLS · SANDBOX · LAB</span>
            </h1>
            <p className="text-xs text-slate-400 mt-1 font-semibold leading-relaxed max-w-4xl">
              Bộ công cụ tương tác phục vụ học tập: kiểm tra chi phí, mô phỏng bút toán, gross-up thuế, audit risk, dòng tiền, SQL và checklist hồ sơ. Không thay thế tư vấn thuế/kế toán chính thức.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="grid lg:grid-cols-12 gap-6">
          ...
          {tools.map(tool => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3.5 rounded-xl border transition-all flex items-start justify-between gap-3 ${activeTool === tool.id ? 'bg-emerald-500/10 border-emerald-500 ring-1 ring-emerald-500/30' : 'bg-[#060a12] border-slate-850 hover:bg-slate-900'}`}
              >
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-center text-emerald-400 shrink-0">
                    <ToolIcon className="w-4 h-4" />
                  </div>
                  <div>
                    <span className="text-[9px] font-black text-slate-500 font-mono">{tool.order} · {tool.badge}</span>
                    <span className="text-xs font-bold text-slate-200 block mt-0.5">{tool.title}</span>
                    <span className="text-[10px] text-slate-500 block mt-0.5 leading-snug">{tool.subtitle}</span>
                  </div>
                </div>
              </button>
            );
        ...
        <main className="lg:col-span-8 bg-slate-950/50 border border-slate-850 rounded-2xl p-5 space-y-5">
          <div className="flex items-start justify-between gap-4 border-b border-slate-850 pb-4">
            <div>
              <span className="text-[9px] font-black text-emerald-400 font-mono uppercase tracking-widest">{selected.order} · {selected.badge}</span>
              <h2 className="text-base font-black text-white mt-1 flex items-center gap-2"><Icon className="w-5 h-5 text-emerald-400" />{selected.title}</h2>
              <p className="text-xs text-slate-400 mt-1 font-semibold">{selected.subtitle}</p>
            </div>
          </div>
...
function ResultCard({ title, children, tone = 'slate' }: { title: string; children: React.ReactNode; tone?: 'slate' | 'emerald' | 'amber' | 'rose' | 'blue' | 'purple' }) {
  const toneClass = {
    slate: 'border-slate-850 bg-slate-900/35 text-slate-300',
    emerald: 'border-emerald-500/25 bg-emerald-950/15 text-emerald-200',
    amber: 'border-amber-500/25 bg-amber-950/15 text-amber-200',
    rose: 'border-rose-500/25 bg-rose-950/15 text-rose-200',
    blue: 'border-blue-500/25 bg-blue-950/15 text-blue-200',
    purple: 'border-purple-500/25 bg-purple-950/15 text-purple-200'
  }[tone];
...
function ExpenseComplianceChecker() {
  ...
  return <div className="space-y-4">
    <div className="grid md:grid-cols-2 gap-4">
      <div><FieldLabel>Loại chi phí</FieldLabel><select value={expenseType} onChange={e => setExpenseType(e.target.value)} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="vat_tu_cong_trinh">Vật tư công trình</option><option value="xang_dau">Xăng dầu / xe máy thi công</option><option value="nhan_cong">Nhân công thuê ngoài</option><option value="tiep_khach">Tiếp khách</option><option value="van_phong">Hành chính văn phòng</option></select></div>
      <div><FieldLabel>Số tiền thanh toán</FieldLabel><input value={amount} onChange={e => setAmount(Number(e.target.value || 0))} type="number" className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs" /></div>
      <div><FieldLabel>Hóa đơn/chứng từ</FieldLabel><select value={hasInvoice ? 'yes' : 'no'} onChange={e => setHasInvoice(e.target.value === 'yes')} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="yes">Có hóa đơn/chứng từ</option><option value="no">Chưa có hóa đơn/chứng từ</option></select></div>
      <div><FieldLabel>Thanh toán</FieldLabel><select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value as 'bank' | 'cash')} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="bank">Chuyển khoản</option><option value="cash">Tiền mặt</option></select></div>
      <div><FieldLabel>Nhà cung cấp</FieldLabel><select value={supplierType} onChange={e => setSupplierType(e.target.value as 'company' | 'individual')} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="company">Công ty/hộ kinh doanh có hóa đơn</option><option value="individual">Cá nhân</option></select></div>
    </div>
    <div className="grid md:grid-cols-3 gap-4">
      <ResultCard title="Điểm rủi ro" tone={tone as 'emerald' | 'amber' | 'rose'}><span className="text-2xl font-black font-mono">{result.risk}/100</span><p className="mt-1 text-[11px]">{result.risk >= 70 ? 'Rủi ro cao' : result.risk >= 40 ? 'Rủi ro vừa' : 'Rủi ro thấp'}</p></ResultCard>
      <ResultCard title="Cảnh báo" tone="amber">{result.issues.length ? <ul className="space-y-1 list-disc pl-4">{result.issues.map(i => <li key={i}>{i}</li>)}</ul> : 'Chưa thấy cảnh báo lớn trong mô phỏng.'}</ResultCard>
      <ResultCard title="Checklist hồ sơ" tone="blue"><ul className="space-y-1 list-disc pl-4">{result.docs.map(doc => <li key={doc}>{doc}</li>)}</ul></ResultCard>
    </div>
  </div>;
}
...
function JournalEntrySimulator() {
  ...
  const templates: Record<string, { title: string; entries: string[]; impact: string }> = {
    material: { title: 'Mua vật tư công trình chưa thanh toán, có VAT', entries: ['Nợ TK 152/156 hoặc 154: Giá chưa thuế', 'Nợ TK 1331: Thuế GTGT được khấu trừ', 'Có TK 331: Tổng thanh toán'], impact: 'Tăng tài sản/chi phí dở dang, tăng VAT đầu vào, tăng phải trả nhà cung cấp.' },
    labor: { title: 'Chi nhân công thuê ngoài cho công trình', entries: ['Nợ TK 154/622: Chi phí nhân công', 'Có TK 111/112/331: Số phải trả', 'Có TK 3335 nếu có khấu trừ thuế TNCN'], impact: 'Tăng chi phí công trình, có thể phát sinh nghĩa vụ khấu trừ thuế.' },
    revenue: { title: 'Nghiệm thu doanh thu công trình', entries: ['Nợ TK 131: Tổng giá thanh toán', 'Có TK 511: Doanh thu', 'Có TK 3331: VAT đầu ra', 'Nợ TK 632 / Có TK 154: Kết chuyển giá vốn'], impact: 'Tăng doanh thu, VAT phải nộp và ghi nhận giá vốn tương ứng.' }
  };
  const item = templates[caseId];
  return <div className="space-y-4"><div><FieldLabel>Nghiệp vụ</FieldLabel><select value={caseId} onChange={e => setCaseId(e.target.value)} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="material">Mua vật tư công trình</option><option value="labor">Nhân công thuê ngoài</option><option value="revenue">Nghiệm thu doanh thu</option></select></div><ResultCard title={item.title} tone="purple"><ul className="space-y-2">{item.entries.map(entry => <li key={entry} className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />{entry}</li>)}</ul></ResultCard><ResultCard title="Ảnh hưởng báo cáo" tone="blue">{item.impact}</ResultCard></div>;
}
...
function CashFlowForecast() {
  ...
  const rows = Array.from({ length: 4 }, (_, index) => {
    const end = opening + (weeklyIn - weeklyOut) * (index + 1);
    return { week: index + 1, in: weeklyIn, out: weeklyOut, end };
  });
...
function PromptBuilder() {
  ...
  return <div className="space-y-4"><div><FieldLabel>Mục tiêu</FieldLabel><input value={goal} onChange={e => setGoal(e.target.value)} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs" /></div><ResultCard title="Prompt đề xuất" tone="purple"><pre className="whitespace-pre-wrap font-mono text-[11px]">{prompt}</pre></ResultCard><button onClick={copy} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-black rounded-xl flex items-center gap-2">{copied ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />}{copied ? 'Đã copy' : 'Copy prompt'}</button></div>;
}

function SqlPlayground() {
  const [query, setQuery] = useState('duplicate');
  const sql: Record<string, string> = {
    duplicate: `SELECT vendor_id, invoice_no, total_amount, COUNT(*) AS count_dup\nFROM expenses\nGROUP BY vendor_id, invoice_no, total_amount\nHAVING COUNT(*) > 1;`,
    aging: `SELECT customer_id, SUM(amount_due) AS balance,\n  CASE WHEN days_overdue > 90 THEN '90+' WHEN days_overdue > 30 THEN '31-90' ELSE '0-30' END AS bucket\nFROM receivables\nGROUP BY customer_id, bucket;`,
    overrun: `SELECT project_id, budget_amount, actual_cost,\n  actual_cost - budget_amount AS variance\nFROM project_costs\nWHERE actual_cost > budget_amount;`
  };
  return <div className="space-y-4"><div><FieldLabel>Bài test SQL</FieldLabel><select value={query} onChange={e => setQuery(e.target.value)} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="duplicate">Duplicate payment</option><option value="aging">Aging công nợ</option><option value="overrun">Vượt ngân sách công trình</option></select></div><pre className="p-4 bg-[#02050b] border border-slate-850 rounded-xl text-[11px] font-mono text-slate-300 overflow-x-auto">{sql[query]}</pre></div>;
}
...
function DocumentChecklistGenerator() {
  ...
  const lists: Record<string, string[]> = {
    material: ['Hợp đồng/đơn đặt hàng', 'Hóa đơn', 'Biên bản giao nhận', 'Phiếu nhập kho', 'Phiếu xuất theo công trình', 'Ủy nhiệm chi/phiếu chi'],
    labor: ['Hợp đồng giao khoán', 'CCCD/MST cá nhân nếu có', 'Bảng chấm công/xác nhận khối lượng', 'Biên bản nghiệm thu', 'Chứng từ khấu trừ thuế nếu có', 'Phiếu chi/ủy nhiệm chi'],
    fuel: ['Hóa đơn xăng dầu', 'Lệnh điều xe', 'Nhật trình xe/máy', 'Bảng kê nhiên liệu', 'Định mức hoặc giải trình sử dụng'],
    revenue: ['Hợp đồng', 'Phụ lục/BOQ', 'Biên bản nghiệm thu', 'Hồ sơ thanh toán', 'Hóa đơn đầu ra', 'Đối chiếu công nợ']
  };
  return <div className="space-y-4"><div><FieldLabel>Loại nghiệp vụ</FieldLabel><select value={type} onChange={e => setType(e.target.value)} className="w-full bg-[#02050b] border border-slate-800 rounded-xl p-2.5 text-xs"><option value="material">Mua/xuất vật tư</option><option value="labor">Nhân công thuê ngoài</option><option value="fuel">Xăng dầu</option><option value="revenue">Nghiệm thu doanh thu</option></select></div><ResultCard title="Checklist hồ sơ" tone="blue"><ul className="space-y-2">{lists[type].map(item => <li key={item} className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />{item}</li>)}</ul></ResultCard></div>;
}
