...
import AccountingTaxBridgeMatrix from './AccountingTaxBridgeMatrix';
import { BookOpen, Bot, Brain, Bug, CheckCircle2, ClipboardCheck, FileCheck2, GitCompare, KeyRound, ListChecks, LockKeyhole, Map, RotateCcw, ShieldCheck, Target, TestTube2, FileSpreadsheet } from 'lucide-react';

type TabKey = 'ai_team' | 'acceptance' | 'qa_matrix' | 'bug_triage' | 'release' | 'rollback' | 'risk_register' | 'security_governance' | 'roadmap_backlog' | 'system_map' | 'month_end_close' | 'ml_explainability' | 'rule_library' | 'bridge_matrix';
...
const tabs: TabItem[] = [
  ...
  { id: 'bridge_matrix', order: '14', title: 'Accounting - Tax Bridge Matrix', subtitle: 'Ma trận kế toán - thuế - hồ sơ - kiểm soát', icon: GitCompare, badge: 'BRIDGE' }
];
...
export default function ProjectControlQALab() {
  ...
  if (activeTab === 'bridge_matrix') return <AccountingTaxBridgeMatrix />;
      ...
      <section className="bg-gradient-to-r from-amber-950/20 via-[#060a12] to-sky-950/20 border border-slate-800 rounded-3xl p-6 relative overflow-hidden">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 font-semibold leading-relaxed max-w-4xl">Phòng lab dành cho người quản lý dự án không chuyên code: giao việc cho AI, nghiệm thu tính năng, test build, phân loại lỗi, release, rollback, backlog, khóa sổ, ML explainability, rule library, bridge matrix và kiểm soát bảo mật dữ liệu.</p>
          </div>
        </div>
      </section>
